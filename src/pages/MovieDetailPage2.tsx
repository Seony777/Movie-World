          ...
          <div className="p-4 h-screen grid grid-rows-2 gap-4">
            ...
            <div className="bg-transparent p-2 min-h-0 overflow-y-auto">
              ...
              <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
                {apiData?.credits.cast.map((casts) => (
                  <PeopleCard
                    key={`cast-${casts.id}`}
                    photo={casts.profile_path}
                    name={casts.name}
                    character={casts.character}
                  />
                ))}
                {apiData?.credits.crew.map((crew) => (
                  <PeopleCard key={`crew-${crew.id}`} photo={crew.profile_path} name={crew.name} character={crew.job} />
                ))}
              </div>
            </div>
          </div>
